...
import { waitingByProject } from '../../../src/agents/model'
          ...
          return (
            ...
            >
              ...
              <View style={{ flex: 1, padding: 14, gap: 3 }}>
                ...
                {stateLabel(session.state.state) === '' && session.agent === undefined ? null : (
                  <Text style={{ color: stateColour(session.state.state), fontSize: 13 }}>
                    {stateLabel(session.state.state)}
                    {session.agent !== undefined
                      ? `${stateLabel(session.state.state) === '' ? '' : ' · '}${session.agent}`
                      : ''}
                  </Text>
                )}
